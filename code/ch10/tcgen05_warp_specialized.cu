/**
 * ch10: True warp-specialized tcgen05 GEMM (producer/consumer).
 *
 * Warp 0 issues TMA loads, warp 1 executes MMA, with a 4-stage pipeline.
 */
...
namespace warp_specialized_impl {
...
gemm_warp_specialized(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  // =================== WARP 1: CONSUMER ===================
    ...
    for (int k = 0; k < num_k_tiles; ++k) {
      ...
      // Wait for producer (lane 0) then sync the MMA warp.
      if (is_lane0) {
      ...
      // Signal stage consumed (lane 0) then sync the MMA warp.
      if (is_lane0) {
...
torch::Tensor run_warp_specialized_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  auto* kernel_ptr = &gemm_warp_specialized<SharedStorageT, decltype(mA), decltype(mB), decltype(mC), decltype(mD),
...
torch::Tensor matmul_tcgen05_warp_specialized(torch::Tensor a, torch::Tensor b) {
  return warp_specialized_impl::run_warp_specialized_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_warp_specialized", &matmul_tcgen05_warp_specialized);
}

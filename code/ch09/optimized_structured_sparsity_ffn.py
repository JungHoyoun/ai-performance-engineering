"""Optimized 2:4 structured-sparse SwiGLU FFN using cuSPARSELt."""
...
class OptimizedStructuredSparsityFFNBenchmark(VerificationPayloadMixin, BaseBenchmark):
    """Structured-sparse FFN optimized with cuSPARSELt."""
    ...
    def setup(self) -> None:
        ...
        self.ffn.setup_sparse()
        self._synchronize()

    def benchmark_fn(self) -> None:
        if self.ffn is None:
            raise RuntimeError("Benchmark not initialized")
        with self._nvtx_range("optimized_structured_sparsity_ffn"):
...
def get_benchmark() -> BaseBenchmark:
    return OptimizedStructuredSparsityFFNBenchmark()

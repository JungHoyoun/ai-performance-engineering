// baseline_cublas_gemm_fp4_perchannel.cu -- NVFP4 GEMM with per-channel output scaling (baseline cuBLASLt)
//
// Baseline uses cuBLASLt NVFP4 with minimal workspace to provide a conservative
// starting point before tuning workspace and heuristics in the optimized variant.
...
constexpr size_t kWorkspaceBytes = 8ull * 1024ull * 1024ull;  // 8MB baseline workspace

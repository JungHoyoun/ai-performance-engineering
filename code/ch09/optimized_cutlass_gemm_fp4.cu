// optimized_cutlass_gemm_fp4.cu -- CUTLASS NVFP4 GEMM optimized (warp-specialized schedule).
...
// The challenge shape has only 112 CTAs along N with the smallest supported TileShape_N=64.
// This example focuses on per-CTA efficiency (TMA warp-specialized schedule) without adding
// additional parallelism (no Split-K / Stream-K) and without CTA clustering.
...
int main() {
    ...
    for (const auto& s : kBenchShapes) {
        ...
        std::cout << "CUTLASS NVFP4 GEMM (optimized) shape=("
    ...
    std::cout << "CUTLASS NVFP4 GEMM (optimized) GEOMEAN_MS: " << geom_ms << " ms" << std::endl;

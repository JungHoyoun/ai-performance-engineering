// optimized_cutlass_gemm_preloaded.cu -- CUTLASS Tensor Core GEMM with preloaded inputs.
//
...
int main() {
    ...
    for (int iter = 0; iter < kIterations; ++iter) {
        NVTX_RANGE("compute_math:cutlass_preloaded_tensorop");
    ...
    std::cout << "CUTLASS Tensor Core GEMM (preloaded inputs): " << avg_ms << " ms" << std::endl;

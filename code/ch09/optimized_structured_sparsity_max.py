"""Optimized max-size GEMM using a 2:4 structured sparse weight matrix."""
...
# B200-tuned cuSPARSELt algorithm parameters for 49,152^2 FP16.
_CUSPARSELT_ALG_ID = 43
_CUSPARSELT_SPLIT_K = 1
_CUSPARSELT_SPLIT_K_MODE = 6


class OptimizedStructuredSparsityMaxBenchmark(VerificationPayloadMixin, BaseBenchmark):
    """Sparse GEMM optimized with 2:4 structured sparsity (max-size shape)."""
    ...
    def benchmark_fn(self) -> None:
        if self.input_t is None or self.weight_compressed is None:
            raise RuntimeError("Benchmark not initialized")
        with self._nvtx_range("optimized_structured_sparsity_max"):
    ...
    def get_config(self) -> BenchmarkConfig:
        return BenchmarkConfig(
            iterations=10,
            warmup=5,
            nsys_nvtx_include=["structured_sparsity_max"],
        )
...
def get_benchmark() -> BaseBenchmark:
    return OptimizedStructuredSparsityMaxBenchmark()

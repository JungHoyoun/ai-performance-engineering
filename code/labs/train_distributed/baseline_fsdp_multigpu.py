...
def main():
    ...
    if rank == 0:
        print("[baseline_fsdp_multigpu] model instantiated", flush=True)
    fsdp_model = _wrap_fsdp(model)
    if rank == 0:
        print("[baseline_fsdp_multigpu] fsdp wrapped", flush=True)
    optimizer = torch.optim.AdamW(fsdp_model.parameters(), lr=args.learning_rate, betas=(0.9, 0.95), weight_decay=0.1)
    if rank == 0:
        print("[baseline_fsdp_multigpu] optimizer ready", flush=True)

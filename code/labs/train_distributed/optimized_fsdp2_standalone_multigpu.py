...
class SimpleTransformerStack(nn.Module):
    def __init__(self, layers: list[SimpleTransformerLayer]):
        super().__init__()
...
def _require_torchao() -> None:
    if convert_to_float8_training is None or Float8LinearConfig is None:
        raise RuntimeError("torchao float8 is required for optimized_fsdp_multigpu2_standalone_multigpu")
...
def _make_fp8_config() -> "Float8LinearConfig":
    ...
    if "enable_pre_and_post_forward" in params:
        kwargs["enable_pre_and_post_forward"] = True
    return Float8LinearConfig(**kwargs)
...
def run_benchmark(**_kwargs) -> dict:
    ...
    raise RuntimeError("optimized_fsdp_multigpu2_standalone_multigpu must be executed via torchrun.")
...
class OptimizedFSDP2Benchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def setup(self) -> None:
        ...
        torch.manual_seed(42)
        torch.cuda.manual_seed_all(42)
        self._model = _build_model(_DEFAULT_HIDDEN, _DEFAULT_LAYERS, self.device).to(torch.bfloat16)
        fp8_config = _make_fp8_config()
    ...
    def get_torchrun_spec(self, config: Optional[BenchmarkConfig] = None) -> TorchrunLaunchSpec:
        ...
        return TorchrunLaunchSpec(
            ...
            script_args=[],
            multi_gpu_required=True,
            name="optimized_fsdp_multigpu2_standalone_multigpu",
            config_arg_map={
                "iterations": "--iters",
                "warmup": "--warmup",
            },

...
# Optional environment overrides so we can explore kernel configs without duplicating this file.
# These are intended for local harness tuning only. For final Popcorn submission, we should
# bake in the chosen configuration.
...
def _env_tuple3_int(name: str, default: tuple[int, int, int]) -> tuple[int, int, int]:
    raw = os.getenv(name)
    if raw is None or raw == "":
        return default
    parts = [p for p in raw.replace(",", " ").split(" ") if p.strip()]
    if len(parts) != 3:
        raise ValueError(f"Expected 3 ints for {name}, got {raw!r}")
    try:
        return (int(parts[0]), int(parts[1]), int(parts[2]))
    except ValueError as e:
        raise ValueError(f"Invalid tuple ints for {name}={raw!r}") from e
...
mma_tiler_mnk = _env_tuple3_int("AISP_NVFP4_GROUP_GEMM_MMA_TILER_MNK", (128, 128, 256))
...
threads_per_cta = _env_int("AISP_NVFP4_GROUP_GEMM_THREADS_PER_CTA", 128)
# Stage numbers of shared memory and tmem
num_acc_stage = _env_int("AISP_NVFP4_GROUP_GEMM_NUM_ACC_STAGE", 1)
num_ab_stage = _env_int("AISP_NVFP4_GROUP_GEMM_NUM_AB_STAGE", 1)
# Total number of columns in tmem
num_tmem_alloc_cols = _env_int("AISP_NVFP4_GROUP_GEMM_TMEM_COLS", 512)
...
def compile_kernel(problem_sizes):
    ...
    # The JIT specialization depends on the per-group (m, n) tiling used to decode blockIdx.z.
    # Keying only by group count can silently reuse a compiled kernel for different shapes and
    # produce incorrect results (e.g., multiple G=8 benchmark cases).
    cache_key = tuple(tuple(int(x) for x in entry) for entry in problem_sizes)

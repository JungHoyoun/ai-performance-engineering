"""CUTLASS NVFP4 grouped GEMM (competition case 0) - 2SM with CUDA Graph replay.

This variant captures steady-state plan replay in setup() and times graph replay in benchmark_fn().
"""
...
# Match current strongest launch defaults for case0.
...
def get_benchmark() -> BaseBenchmark:
    ...
    bench = NVFP4GroupGemmBenchmark(
        ...
        prepare=prepare_cutlass_cached_2sm_graph,
        inputs_per_iteration=15,
        name=f"nvfp4_group_gemm_{case.name}_optimized_cutlass_cached_2sm_graph",
    )

"""CUTLASS NVFP4 grouped GEMM (competition case 0) - 2SM N32 persistent candidate.

This variant increases CTA count via an N=32 tile while preserving the same workload
and persistent fused-request scheduling.
"""
...
def get_benchmark() -> BaseBenchmark:
    ...
    bench = NVFP4GroupGemmBenchmark(
        ...
        prepare=prepare_cutlass_cached_2sm_n32_persistent,
        inputs_per_iteration=15,
        capture_iter_graph=False,
        name=f"nvfp4_group_gemm_{case.name}_optimized_cutlass_cached_2sm_n32_cm2_cn1_ro2_pdl1_persistent",
    )

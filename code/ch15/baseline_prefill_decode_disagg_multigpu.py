#!/usr/bin/env python3
"""baseline_prefill_decode_disagg_multigpu.py - CPU-staged KV handoff baseline.

Models disaggregated prefill/decode across GPU pairs:
- Prefill runs on even-index GPUs
- Decode runs on the next odd-index GPU
- KV handoff is staged through host memory
"""
...
class BaselinePrefillDecodeDisaggBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def benchmark_fn(self) -> None:
        ...
        with self._nvtx_range("baseline_prefill_decode_disagg_multigpu"):
            with torch.no_grad():
                for (prefill_id, decode_id), prefill_model, decode_model, batch in zip(
                    ...
                    for idx in range(batch.shape[0]):
                        ...
                        kv_decode = kv_cpu.to(decode_device)

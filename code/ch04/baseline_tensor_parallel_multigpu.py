...
def _resolve_world_size() -> int:
    ...
    if world_size < 2:
        raise RuntimeError("baseline_tensor_parallel_multigpu requires >=2 GPUs.")
    return world_size
...
def _init_distributed() -> tuple[int, int, int]:
    if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
        raise RuntimeError("baseline_tensor_parallel_multigpu requires torchrun (RANK/WORLD_SIZE missing).")
...
class BaselineTensorParallelBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def get_torchrun_spec(self, config: Optional[BenchmarkConfig] = None) -> TorchrunLaunchSpec:
        ...
        return TorchrunLaunchSpec(
            ...
            script_args=[],
            multi_gpu_required=True,
            name="baseline_tensor_parallel_multigpu",
            config_arg_map={
                "iterations": "--iters",
                "warmup": "--warmup",
            },

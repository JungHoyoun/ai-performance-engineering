// Optimized loop-unrolling binary: ILP-friendly kernel with vectorized loads.
...
int main() {
    ...
    for (size_t i = 0; i < input_elements; ++i) {
        NVTX_RANGE("setup");
        h_inputs[i] = static_cast<float>((i % 2048) - 1024) / 1024.0f;
    }
    for (int i = 0; i < kWeightPeriod; ++i) {
        NVTX_RANGE("setup");
        h_weights[i] = 0.25f + 0.05f * static_cast<float>(i);
    }
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        launch_loop_unrolling_optimized(d_inputs, d_weights, d_output, rows, nullptr);
    }
    ...
    std::cout << "Unrolled loop (optimized): " << avg_ms << " ms\n";
